...
sealed class VaultSaveResult {
    data object Saved : VaultSaveResult()
    data object AuthenticationRequired : VaultSaveResult()
    data object KeyInvalidated : VaultSaveResult()
    data class Failed(val message: String) : VaultSaveResult()
}
...
class PrivateMemoryVault(
    ...
    /** Keeps the vault encrypted while making expected Keystore failures safe for UI callers. */
    @Synchronized
    fun saveSafely(memory: PrivateMemory): VaultSaveResult {
        return try {
            save(memory)
            VaultSaveResult.Saved
        } catch (_: android.security.keystore.UserNotAuthenticatedException) {
            VaultSaveResult.AuthenticationRequired
        } catch (_: KeyPermanentlyInvalidatedException) {
            // Do not silently replace a key: that would make existing private data unrecoverable.
            VaultSaveResult.KeyInvalidated
        } catch (error: Exception) {
            VaultSaveResult.Failed(error.message ?: "Unable to encrypt private data")
        }
    }
    ...
    /** Checks enrollment/configuration presence without decrypting the protected value. */
    @Synchronized
    fun contains(canonicalKey: String): Boolean {
        migrateLegacyMemoriesIfNeeded()
        database.readableDatabase.query(
            TABLE_MEMORIES,
            arrayOf(COLUMN_ID),
